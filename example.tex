...
$f = x^n - p$.
This polynomial is irreducible over $\Z$ by Eisenstein's criterion, and
so it is irreducible over $\Q$ by Gauss's Lemma.
In particular, $f$ has no roots in $\Q$, there is no rational number
...
Let $x \in \mathbb R$ be given, and choose a sequence $\seq{x_n}$ in $S$ that
converges to $x$.
